...
class LyricHelper {
  static double getTotalHeight(List<LyricsLineModel>? lyrics, int playingIndex, LyricUI ui) {
    return getLyricHeight(lyrics, playingIndex) + getSpaceHeight(lyrics, ui);
  }

  ///获取歌词整体高度
  static double getLyricHeight(List<LyricsLineModel>? lyrics, int playingIndex) {
    if (lyrics == null) {
      return 0;
    }
  ...
  static double centerOffset(LyricsLineModel? lyric, bool isCurr, LyricUI lyricUI, int playIndex) {
    ...
    switch (baseLine) {
      ...
      case LyricBaseLine.EXT_CENTER:
        ...
        return ((isCurr
            ...
            ((isCurr ? lyric.drawInfo?.playingExtTextHeight : lyric.drawInfo?.otherExtTextHeight) ??
                    0) /
                2;
