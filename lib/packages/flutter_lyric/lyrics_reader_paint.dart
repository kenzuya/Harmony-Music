...
import 'package:harmonymusic/packages/flutter_lyric/lyric_helper.dart';
import 'package:harmonymusic/packages/flutter_lyric/lyric_ui/lyric_ui.dart';
import 'package:harmonymusic/packages/flutter_lyric/lyrics_log.dart';
...
class LyricsReaderPaint extends ChangeNotifier implements CustomPainter {
  ...
  double drawLine(int i, double drawOffset, Canvas canvas, LyricsLineModel element) {
    //空行直接返回
  ...
  double _drawOtherLyricLine(
    ...
    var extTextPainter =
        (isPlay ? element.drawInfo?.playingExtTextPainter : element.drawInfo?.otherExtTextPainter);
    //该行行高
  ...
  void drawHighlight(LyricsLineModel model, Canvas canvas, TextPainter? painter, Offset offset) {
    ...
    model.drawInfo?.inlineDrawList.forEach((element) {
      ...
      canvas.drawRect(
          Rect.fromLTWH(dx, offset.dy + element.offset.dy - 2, currentWidth, element.height + 2),
  ...
  double drawText(Canvas canvas, TextPainter? paint, double offsetY, [LyricsLineModel? element]) {
    //paint 理论上不可能为空，预期报错
